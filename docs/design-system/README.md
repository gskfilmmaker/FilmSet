...
## Script Import auto-populates Cast, not just Locations

Locations got populated as a side-effect of a required foreign key (`scenes.location_id` is `NOT NULL`); Cast was previously silently discarded even though the parser already detects character cues (needed to render dialogue correctly). Both `importScript` and `importRevision` now also call the new `charactersInScene` (`apps/web/lib/script-parser.ts`) for every scene they touch, and for each unique character name: `findOrCreateCharacter` → `findOrCreateCastMember` (`apps/web/lib/find-or-create.ts`, new sibling of the location helper — opens a cast slot with no actor attached yet, status "Offer Out"/contract "Missing", so it shows up on `/cast` as needing casting) → link into `scene_cast` (`onConflictDoNothing`, so re-importing is idempotent). A revision import only ever *adds* links, never removes one — a manual cast assignment always survives a re-import. Crew still can't be derived this way — nothing in a screenplay says who's on crew.

## Delete + Undo — every list is now add/edit/delete, not just add/edit

Cast, Crew, and Locations already had inline add/edit; delete existed but was a single irreversible click. Their `onDelete` now captures the row's field values before deleting and shows a toast (`ToastAction` from `@filmset/ui`) with an "Undo" button that re-creates it via the same `create*` action — cheap because these are flat rows with no cascading children. This is app code, not a new primitive (`packages/ui`'s `toast()` already accepted an `action` node; it just wasn't used anywhere yet).

Scenes and Shoot Days could be created and edited but never deleted at all — a real gap once a duplicate or mis-imported scene needed removing. Both now have a **hard delete**, reached via a "Delete" button in their edit Inspector, behind `ConfirmDeleteDialog` (`apps/web/components/confirm-delete-dialog.tsx`, thin wrapper on `packages/ui`'s `Dialog` — its own doc comment already reserves it for exactly this: irreversible action confirmation). Undo isn't offered here on purpose: deleting a scene cascades its script pages and breakdown links, deleting a shoot day cascades its call sheet, and silently losing that on an accidental Undo-miss would be worse than asking first.

- `deleteScene` (`apps/web/app/script/scene-actions.ts`) — a true hard delete for a scene that was never really shot (duplicate, added by mistake). It never renumbers other scenes — locked scene numbers stay stable, matching `importRevision`'s own rule. A scene that's part of the record but won't be shot should be marked **Omitted** via edit instead, not deleted.
- `deleteShootDay` (`apps/web/app/schedule/shoot-day-actions.ts`) — its call sheet cascades; any scene scheduled on it falls back to unscheduled (`ON DELETE SET NULL`, never deleted). Remaining days are renumbered by date afterward so "Day X of N" stays contiguous, the same invariant `createShootDay` already keeps.
...
8. **Cast from script + delete/undo** — after importing a script with dialogue, check `/cast`: every speaking character should already be listed, showing "Not yet cast" until you assign an actor. On `/cast`, `/crew`, or `/locations`, delete a row and click "Undo" in the toast before it disappears — confirm the row comes back. On `/script`, edit a scene and click "Delete" — confirm the confirmation dialog, then that the scene (and its script page) is gone and other scene numbers didn't shift. On `/schedule`, edit a shoot day and delete it — confirm its scenes land back in Unscheduled and the remaining days renumber.
