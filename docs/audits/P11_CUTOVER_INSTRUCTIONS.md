# Instructions — FilmSet P11 Live Supabase Cutover (0021)
...
Same shape as the P10 cutover: **purely additive, touches no existing data.** It creates six brand-new, empty tables for a new Transportation feature (vehicles, drivers, driver qualifications, movements, movement legs, and passenger manifests), plus the RLS permissions those tables need. No row in any existing table is read, changed, or deleted.

## What this adds

A real Transportation section (now in the left sidebar, next to Accommodation) for booking vehicles and drivers to move cast/crew — create vehicles and drivers, plan a "movement" (e.g. an airport pickup), add legs with pickup/dropoff, a scheduled time, an assigned vehicle and driver, and a passenger list.

This is completely separate from today's existing shoot-day vehicle/transport-run fields on the call sheet — those are untouched.
...
```
ALL P11 CUTOVER INVARIANTS PASSED (0021) -- safe to commit.
```
...
If the prerequisite migrations aren't live yet (0016-0020, from the earlier cutovers), or this script has already been run, it raises `PRE-FLIGHT ABORT:` naming exactly what didn't match, and Postgres automatically rolls back the entire transaction — nothing is left half-applied. Tested against real PostgreSQL 16: a full successful run, a re-run against an already-migrated database, and a run against a database missing the prerequisites.
...
```sql
-- ============================================================================
-- FilmSet -- P11 Cutover Script (0021)
-- Generated to accompany the Transportation domain PR
-- (claude/p11-transportation-domain).
-- Applies migration 0021_transportation_domain.sql against live Supabase.
--
-- Purely additive, same posture as the P10 cutover: no existing data is
-- read, changed, or deleted. New, empty tables plus RLS policies only.
-- The pre-flight below confirms the prerequisite migrations (through
-- 0020) are already live, and that this script hasn't already been run,
...
do $preflight$
begin
  if not exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'accommodation_properties') then
    raise exception 'PRE-FLIGHT ABORT: public.accommodation_properties not found -- migration 0020 does not appear to be live yet. This script depends on 0016-0020 already being applied.';
  end if;
  ...
  if not exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'locations') then
    raise exception 'PRE-FLIGHT ABORT: public.locations not found -- this should always exist from the base schema. Something is very wrong; aborting.';
  end if;

  if exists (select 1 from information_schema.tables where table_schema = 'public' and table_name = 'vehicles') then
    raise exception 'PRE-FLIGHT ABORT: public.vehicles already exists -- migration 0021 appears to already be live. Not safe to run this script again.';
  end if;

  raise notice 'Pre-flight checks passed: 0016-0020 are live, 0021 is not yet applied. Proceeding.';
end $preflight$;

-- ============================================================================
-- BEGIN packages/db/migrations/0021_transportation_domain.sql (pasted verbatim, unmodified)
-- ============================================================================
...
-- ============================================================================
-- END packages/db/migrations/0021_transportation_domain.sql
-- ============================================================================
...
declare
  ...
  v_vehicle_rows int;
  v_driver_rows int;
  v_qualification_rows int;
  v_movement_rows int;
  v_leg_rows int;
  v_passenger_rows int;
  v_write_policy_tables int;
begin
  select count(*) into v_new_tables
  from information_schema.tables
  where table_schema = 'public'
    and table_name in ('vehicles', 'drivers', 'driver_qualifications', 'movements', 'movement_legs', 'movement_leg_passengers');
  if v_new_tables <> 6 then
    raise exception 'INVARIANT FAILED: expected 6 new transportation tables, found %', v_new_tables;
  end if;

  select count(*) into v_vehicle_rows from public.vehicles;
  select count(*) into v_driver_rows from public.drivers;
  select count(*) into v_qualification_rows from public.driver_qualifications;
  select count(*) into v_movement_rows from public.movements;
  select count(*) into v_leg_rows from public.movement_legs;
  select count(*) into v_passenger_rows from public.movement_leg_passengers;
  if v_vehicle_rows <> 0 or v_driver_rows <> 0 or v_qualification_rows <> 0 or v_movement_rows <> 0 or v_leg_rows <> 0 or v_passenger_rows <> 0 then
    raise exception 'INVARIANT FAILED: all six new transportation tables should be empty immediately after this migration, found % vehicles, % drivers, % qualifications, % movements, % legs, % passengers',
      v_vehicle_rows, v_driver_rows, v_qualification_rows, v_movement_rows, v_leg_rows, v_passenger_rows;
  end if;

  select count(distinct tablename) into v_write_policy_tables
  from pg_policies
  where schemaname = 'public'
    and tablename in ('vehicles', 'drivers', 'driver_qualifications', 'movements', 'movement_legs', 'movement_leg_passengers')
    and cmd = 'ALL';
  ...
  raise notice 'ALL P11 CUTOVER INVARIANTS PASSED (0021) -- safe to commit.';
end $postcheck$;
