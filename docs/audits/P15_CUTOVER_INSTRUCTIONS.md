# Instructions — FilmSet P15 Live Supabase Cutover (0024)
...
Catering domain, phase 2 — the owner asked for "a vendor list, menu list, each cast and crew preferences like vegetarian/non-vegetarian or food preferences and beverage preferences, and any other detail as per the best international catering standards — packaging, timing, hospitality like a five star catering service would do." The vendor list already existed; this migration adds the rest.

**Unlike the P10–P14 cutovers, this one is not purely additive-tables-only.** It creates two brand-new, empty tables (`menu_items`, `catering_order_items`), **and** it adds nullable columns to two existing tables (`dietary_profiles`, `meal_services`) via `ALTER TABLE ADD COLUMN`. This is still safe: every new column is nullable with no default requirement, so existing rows in those two tables simply get `NULL` for the new columns. No existing row is read, rewritten, backfilled, or deleted.

## What this adds

- **Menu catalog** (`menu_items`) — a production's approved dishes: name, category (starter/main/dessert/beverage/snack/bread/side), cuisine, diet type (vegetarian/non-vegetarian/vegan/eggetarian/jain), spice level, packaging type, price + currency, optionally linked to a catering vendor. Same shape/role as the Equipment domain's approved catalog (0023).
- **Order itemization** (`catering_order_items`) — a catering order can now name specific menu items and quantities, not just a vendor.
- **Food & beverage preferences** on `dietary_profiles` — `diet_type` (adds HALAL/KOSHER on top of the menu's five, since a person's own diet identity is broader than one dish's classification), `beverage_preference` (free text), `spice_preference`. These are standing preferences, kept separate from the existing `dietary_requirements` table, which stays exactly as-is for graded allergy severities (Preference/Mild/Severe) — "vegetarian" isn't an allergy.
- **Hospitality / service-standard fields** on `meal_services` — `service_style` (buffet/plated/packed boxes/family style), `packaging_type`, `service_time` (free-text clock time — the row's own `date` already carries the day), `headcount_confirmed`, `hospitality_notes`.

## Before you run anything

1. **Confirm you are connected to the correct Supabase project.**
2. **Run this during low traffic** if convenient — though the risk is low (new nullable columns, new empty tables).
...
```
ALL P15 CUTOVER INVARIANTS PASSED (0024) -- safe to commit.
```
...
If the prerequisite migrations aren't live yet (0022 Catering domain, 0023 Equipment domain), or this script has already been run (fully or partially), it raises `PRE-FLIGHT ABORT:` naming exactly what didn't match, and Postgres automatically rolls back the entire transaction — nothing is left half-applied. Tested against real PostgreSQL 16: a full successful run against a database with 0000–0023 already applied, a re-run against an already-migrated database (correctly aborts), a run against a database missing the Equipment domain prerequisite (correctly aborts), and a post-cutover functional check — a valid menu item insert with every new column populated succeeds, and an invalid enum value (bad category, bad diet type) is correctly rejected by the new check constraints.
