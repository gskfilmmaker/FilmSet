...
import { castMembers, issues, locations, shootDays, theBandScenes } from "@filmset/db";
import { Button, Inspector, InspectorSection, StatusBadge } from "@filmset/ui";
import { Sparkles, Undo2 } from "lucide-react";
import { useSearchParams } from "next/navigation";
import * as React from "react";

const castMemberCharacterIds = Object.fromEntries(castMembers.map((c) => [c.id, c.characterId]));
const scenesById = new Map(theBandScenes.map((s) => [s.id, s]));
const conflictSceneIds = new Set(issues.flatMap((i) => i.affectedSceneIds));
      ...
      inspector={
        primarySelected ? (
          ...
          >
            ...
            <InspectorSection label="Cast">{castNames(primarySelected, castMemberCharacterIds) || "—"}</InspectorSection>
          ...
          <DragOverlay>
            {activeScene ? (
              <div className="w-[600px] rounded-md border border-[var(--color-action-primary)]">
                <Strip
                  scene={activeScene}
                  castLabel={castNames(activeScene, castMemberCharacterIds)}
                  selected={false}
